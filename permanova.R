...
piska_df_species_2007 <- piska_df_2007[,10:colnr]
piska_df_env_2011 <- piska_df_2011[,1:5]
piska_df_species_2011 <- piska_df_2011[,10:colnr]
piska_df_env_2013 <- piska_df_2013[,1:5]
piska_df_species_2013 <- piska_df_2013[,10:colnr]
piska_df_env_2015 <- piska_df_2015[,1:5]
piska_df_species_2015 <- piska_df_2015[,10:colnr]
piska_df_env_2019 <- piska_df_2019[,1:5]
piska_df_species_2019 <- piska_df_2019[,10:colnr]
piska_df_env_2020 <- piska_df_2020[,1:5]
piska_df_species_2020 <- piska_df_2020[,10:colnr]
piska_df_env_2021 <- piska_df_2021[,1:5]
piska_df_species_2021 <- piska_df_2021[,10:colnr]
...
write.csv(fullpic, "permanova detailed results.csv")
...
fullpic_ctrlmg <- filter(fullpic, pairs == "control vs managed")
fullpic_ctrlunmg <- filter(fullpic, pairs == "control vs unmanaged")
fullpic_mgunmg <- filter(fullpic, pairs == "unmanaged vs managed")

summary(lm(F.val ~ year_since_dist, data=fullpic_ctrlmg))
summary(lm(F.val ~ year_since_dist, data=fullpic_ctrlunmg))
summary(lm(F.val ~ year_since_dist, data=fullpic_mgunmg))
